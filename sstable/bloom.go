...
// BuildBloomFilter creates a bloom filter for the given set of keys.
// Returns nil if keys is empty. A nil filter is safe to pass to
// BloomMayContain, which will return true (assume key may be present).
//
// In practice, this is never called with empty keys because the
...
func BuildBloomFilter(keys [][]byte) []byte {
	...
	// Optimal k = bitsPerKey * ln(2) ≈ 6.93
...
// bloomHash is a murmur-inspired hash function.
